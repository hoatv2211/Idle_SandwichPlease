...
public class ModalObject
{
    ...
    private Tuple<ModalCreatorItem, RectTransform>[] bottomPrefab = null;
    ...
    private Stack<RectTransform> stackedPopup;
    public Stack<RectTransform> StackedPopup
    {
        get
        {
            return stackedPopup;
    ...
    public Tuple<ModalCreatorItem, RectTransform>[] BottomPrefab
    {
        get
        {
    ...
    public void OnInitCreatorItems()
    {
        ...
        var bottom = new List<Tuple<ModalCreatorItem, RectTransform>>();
        string prefabname;
            ...
            if (item != null)
            {
                ...
                switch (it.Location)
                {
                    case ModalCreatorItem.PrefabLocationEnum.Button:
                        bottom.Add(Tuple.Create(it, item));
                        break;
    ...
    public ModalObject PushPopup(float popupAfterSeconds = 0.0f)
    {
        ...
        this.stackedPopup = this.stackedPopup ?? new Stack<RectTransform>();
    ...
    System.Action<ModalComponent> _onClick;
    public ModalObject OnButtonClick(System.Action<ModalComponent> onClick)
    {
        this._onClick = onClick;
        return this;
    }

    public System.Action<ModalComponent> GetOnButtonClick()
    {
        return _onClick;
    }
    ...
    private void UpdateText(TMPro.TextMeshProUGUI textItem, string data)
    {
        try
        {
            var result = data;
        ...
        this.OnInit((component, OnSuccess, OnFailure) =>
            {
                for (int i = 0; i < setting.Length; i += 2)
                {
                    ...
                    {
                        if ((comp.GetComponent<TMPro.TextMeshProUGUI>()) != null)
                        {
                            UpdateText(comp.GetComponent<TMPro.TextMeshProUGUI>(), setting[i + 1]);
                        }
                        else if ((t = comp.GetComponent<UnityEngine.UI.Text>()) != null)
                        {
    ...
    {
        var obj = new ModalObject(ModalCreatorPrefabsName.ModalFlexibleCreator);
        ...
        )
    {
        ...
        {
            onOK?.Invoke(result);
        }, onCancel, showPopup, isCustomAnim, button);
        ...
        p.OnInit((component, OnSuccess, OnFailure) =>
        {
            ...
            prev?.Invoke(component, OnSuccess, OnFailure);
        });
        ...
        params ExtraButtonData[] extraButton)
    {
        ...
        {
            setting.Add(ModalConstant.Content);
            setting.Add(content);
        }
        ...
        if (isCustomAnim)
        {
            p.OnModalAppear(ModalControllerAnimation.ShowModalAnim)
                .OnModalDisapppear(ModalControllerAnimation.HideModalAnim);
        }
    ...
    public ModalObject SetBackgroundColor(Color color)
    {
        ModalController.Instance.SetBlockingColor(color);
        return this;
    }

    public ModalObject DisableBlocking()
    {
        ModalController.Instance.DisableBlocking();
        return this;
    }
}
